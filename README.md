...
![Latest Release](https://img.shields.io/github/v/release/mlmateos/texstudio-qt6-builds?include_prereleases&label=latest)
...
The easiest and most secure way to install TeXstudio and keep it automatically updated is via our GPG-signed APT repository.

### Step 1: Import the GPG Key
Our repository is cryptographically signed to ensure package integrity and security.
```bash
sudo mkdir -p /etc/apt/keyrings
curl -fsSL https://raw.githubusercontent.com/mlmateos/texstudio-qt6-builds/master/texstudio-qt6-key.asc | sudo gpg --dearmor -o /etc/apt/keyrings/texstudio-qt6-key.gpg
sudo chmod a+r /etc/apt/keyrings/texstudio-qt6-key.gpg
```
...
```bash
echo "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/texstudio-qt6-key.gpg] https://mlmateos.github.io/texstudio-qt6-builds alpha main" | \
  sudo tee /etc/apt/sources.list.d/texstudio-qt6-builds.list
```
...
sudo rm /etc/apt/sources.list.d/texstudio-qt6-builds.list
```
...
| `--yes` | No confirmation prompts (automated mode) — `.deb` only |
...
./build-texstudio-deb.sh --branch 4.9.6 --clean --poppler --sign --publish
...
| Type | Version | Branch |
| --- | --- | --- |
| 🟢 **Stable** | **4.9.8** | `stable`, `alpha` |
| 🟡 Development | 4.9.9-beta2 | `alpha` |
<!-- AUTO-VERSIONS:END -->
---
...
curl -fsSL https://raw.githubusercontent.com/mlmateos/texstudio-qt6-builds/master/texstudio-qt6-key.asc | gpg --dearmor > texstudio-qt6-key.gpg
